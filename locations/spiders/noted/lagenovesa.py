...
class LagenovesaSpider(scrapy.Spider):
    ...
    download_delay = 1.5
    start_urls = (
        'http://lagenovesasuper.com.ar/index.php/empresa/sucursales',
    )
    ...
    def parse(self, response):
        stores = response.xpath('//div[@class="uk-width-medium-1-2"]/div[@class="uk-panel uk-panel-box"]|//div[@class="uk-width-medium-1-3"]/div[@class="uk-panel uk-panel-box"]')
        map_data = response.xpath('normalize-space(//div[@class="wk-map wk-map-default"]/@data-options)').extract_first()
