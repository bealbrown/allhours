...
class MarketBasketSpider(scrapy.Spider):
    ...
    def parse_stores(self, response):
        map_data = re.findall(r'pois\":\[{\"point\":{\"lat\":[^}]+' , response.body_as_unicode())
        if(len(map_data)==0):
            return
        location = re.findall(r"[-.0-9]+" ,map_data[0])
        address =  response.xpath('normalize-space(//div[@itemtype="http://schema.org/Place"]/div[1]/p[contains(text() ,"Address:")]/text())').extract_first()
