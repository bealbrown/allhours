...
class MilanesaSpider(scrapy.Spider):
    ...
    def parse(self, response):
        stores = response.xpath(
            '//*[@id="stores-address"]/footer/a[1]/@href').extract()
        for store in stores:
        ...
        next_page_url = response.xpath(
            '//*[@id="nav-delivery"]//descendant::*/@href').extract()
        ...
        if next_page < len(continued_urls):
            yield scrapy.Request(
                continued_urls[next_page], callback=self.parse)


    def convert_days(self, days):
        # lun mar mié jue vie sáb dom / mon - sun

        span_eng = {'lun': 'Mo', 'mar': 'Tu', 'mié': 'We', 'jue': 'Th',
    ...
    def parse_store(self, response):
        ref = response.xpath(
            '//*[@id="profile"]/p/span[1]/span[3]/text()').extract_first()

        street = response.xpath(
            '//*[@id="profile"]/p/span[1]/span[2]/text()').extract_first()

        city = response.xpath(
            '//*[@id="profile"]/p/span[1]/span[4]/text()').extract_first()

        # Fix for "C?rdoba"
        city = city.replace('�', 'o')

        postcode = response.xpath(
            '//*[@id="profile"]/p/span[1]/span[5]/text()').extract_first()

        country = response.xpath(
            '//*[@id="profile"]/p/span[1]/span[6]/text()').extract_first()

        name = response.xpath(
            '//*[@id="profile"]/p/span[1]/span[1]/a/text()').extract_first(
        ).strip()

        phone = response.xpath(
            '//*[@id="profile"]/p/span[2]/a/span/text()').extract_first()

        days = self.convert_days(response.xpath(
            '//*[@id="profile"]/p/span[3]/text()').extract_first().strip(
            'Abierto ').split(': ')[0])
        ...
        address = "{} {}, {}, {}".format(street, city, country, postcode,)
