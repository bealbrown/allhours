...
class BobsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        names = response.xpath(
            ...
            '., "MAP")]/descendant-or-self::*/text()').re(regex_name)

        streets = response.xpath(
            '//td[contains(@style,"300px") and contains('
            '., "MAP")]/descendant-or-self::*/text()').re(regex_street)

        phones = response.xpath('//td[contains(@style,"300px") and contains('
                                '., "MAP")]/descendant-or-self::*/text()').re(
                                regex_phone)
        ...
        stores = response.xpath(
            '//td[contains(@style,"300px") and contains(., "MAP")]')
