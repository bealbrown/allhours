...
class SeriousCoffeeSpider(scrapy.Spider):
    ...
    download_delay = 0.5
    start_urls = (
        'http://www.seriouscoffee.com/locations/',
    )
    ...
    def parse_stores(self, response):
        address_list =  response.xpath('//div[@class="cms_locations_mapaddress"]/div[@class="cms_locations_addressphone"]/div[@class="location_address"]/text()').extract()
        ...
        if (len(address_list) > 2):
            postcode = address_list[2].split(",")[1].lstrip().split(" ")[2].strip()+" "+address_list[2].split(",")[1].lstrip().split(" ")[3].strip()
        else:
            postcode = address_list[1].split(",")[1].lstrip().split(" ")[2].strip()+" "+address_list[1].split(",")[1].lstrip().split(" ")[3].strip()
        ...
        properties = {
            ...
            'phone': response.xpath('normalize-space(//div[@class="cms_locations_mapaddress"]/div[@class="cms_locations_addressphone"]/div[@class="location_contact"]/text())').extract_first().replace('\xa0 ' ,''),
            'city': city,
            'state':state,
            'postcode': postcode,
