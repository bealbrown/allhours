...
class BobsSpider(scrapy.Spider):
    ...
    def parse_store(self, response):
        ...
        for i in container:
            ...
            if i.xpath('.//div/ul/li[2]/p/text()').extract_first():
                phone = i.xpath('.//div/ul/li[1]/p/text()')\
                    .extract_first().strip()
            else:
                phone = ''

            if i.xpath('.//div/ul/li[2]/p/a/@href').extract_first():
                lat = i.xpath('.//div/ul/li[2]/p/a/@href').extract_first(
                    ).split('=')[2].split(',')[0]
                lon = i.xpath('.//div/ul/li[2]/p/a/@href').extract_first(
                    ).split('=')[2].split(',')[1]
            else:
                lat = i.xpath('.//div/ul/li/p/a/@href').extract_first(
                ).split('=')[2].split(',')[0]
                lon = i.xpath('.//div/ul/li/p/a/@href').extract_first(
                ).split('=')[2].split(',')[1]
    ...
    def parse(self, response):
        base_url = "http://www.coffeetime.com/locations.aspx?" \
