...
class MightytacoSpider(scrapy.Spider):
    ...
    def parse(self, response):
        stores = response.xpath('//div[@class="contentBlocks"]/div[@class="contentBlock animate clear"]/div[@class="float right half copy animate group"]/div')
        ...
        for store in stores:
            ...
            properties = {
                ...
                'phone':store.xpath(
                    'normalize-space(./div/div[@class="gets"]/div[@class="phoneRow clear"]/p/a[@data-show-device="mobile"]/text())').extract_first(),
                'city': store.xpath(
                    'normalize-space(./div/div[@class="address"]/p/text())').extract_first().strip().split(',')[1],
                'state': store.xpath(
            ...
            hours = self.parse_hours(store.xpath('./div/div[@class="hours"]/div[contains(@id  , "DiningRoomHours")]/p[@class="time"]/text()').extract())
            raw = store.xpath('./div/div[@class="hours"]/div[contains(@id  , "DiningRoomHours")]/p[@class="time"]/text()').extract()
